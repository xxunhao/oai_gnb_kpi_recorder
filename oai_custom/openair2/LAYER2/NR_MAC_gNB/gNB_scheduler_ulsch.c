...
int init_ul_slice_policies() {
  const char *config_path = "../../../rrmPolicy_uplink.json";
                  ...
                  uint16_t *rballoc_mask)
{
  ...
  const int min_rb = 10; //original is 10,lyg change
  ...
  while (remainUEs > 0 && n_rb_sched >= min_rb && iterator->UE != NULL) {
    ...
    printf("[UL PUSCH] UE 0x%04x: scheduled PRBs = %d\n", iterator->UE->rnti, rbSize);
