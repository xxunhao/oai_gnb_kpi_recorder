...
size_t o1_copy_mac_stats_csv(gNB_MAC_INST *gNB, char *output, size_t strlen, bool reset_rsrp)
{
  ...
  const char *filepath = "../../../rrmPolicy_uplink.json";
  struct json_object *json_obj = json_object_from_file(filepath);
  if(!json_obj){
    printf("Error reading json file \n");
  }
  int slice2_max_ratio = 0;
  struct json_object *slice2_obj = NULL;
  struct json_object *max_ratio_obj = NULL;
  if (json_object_object_get_ex(json_obj, "slice2", &slice2_obj) &&
      json_object_object_get_ex(slice2_obj, "max_ratio", &max_ratio_obj)) {
    slice2_max_ratio = json_object_get_int(max_ratio_obj);
  }
  ...
  UE_iterator(gNB->UE_info.list, UE) {
    ...
    output += snprintf(output,
            ...
            "%ld,%d,%d,%s,%04x,%d,%d,%f,%f, ,%d,%d,%d,%d,%d,%d,%u,%u,%d,%d,%"PRIu32",%"PRIu16", ,%.5f,%d,%"PRIu64",%"PRIu64",%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu64",%d,%d,%02d%02d%02d,%"PRIu64",%d,%d,%02d%02d%02d, ,%.5f,%d,%"PRIu64",%"PRIu64",%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu32",%d,%d, ,\n",
            timestamp,
            gNB->frame,
            gNB->slot,
            ue_imsi,
...
void *nrmac_stats_thread(void *arg)
{
  ...
  char ue_metrics_csv_filepath[] = "/home/lyg22/ros2_kpi_toolkit/gnb_csv/gnb_metrics.csv";

  char csv_header[] = "Timestamp, Frame, Slot, IMSI, RNTI, UID, UE_No, DL_THR, UL_THR, ,PH_dB, PCMAX_dBm, AVG_RSRP, CQI, RI, raw_rssi, ul_rssi, dl_max_mcs, sched_ul_bytes, estimated_ul_buffer, num_total_bytes, dl_pdus_total, ,dl_BLER, dl_MCS, dlsch_errors, dlsch_total_bytes, dlsch_current_bytes, dlsch_total_rbs, dlsch_current_rbs, dl_num_mac_sdu, dl_lc4_bytes, dl_lc4_id, dl_lc4_sst,dl_lc4_sd, dl_lc5_bytes, dl_lc5_id, dl_lc5_sst,dl_lc5_sd, ,ul_BLER, ul_MCS, ulsch_errors, ulsch_total_bytes, ulsch_current_bytes, ulsch_total_rbs, ulsch_current_rbs, ul_num_mac_sdu, SNR, slice2_max_ratio, ,\n";
  ...
  while (oai_exit == 0) {
    char *p = output;
    p += o1_copy_mac_stats_csv(gNB, p, end - p, false);
